...
import 'package:flutter_prj/services/audio_party_service.dart';
...
class AudioPartyScreen extends StatefulWidget {
  final String? partyId;

  const AudioPartyScreen({super.key, this.partyId});

  @override
  State<AudioPartyScreen> createState() => _AudioPartyScreenState();
}

class _AudioPartyScreenState extends State<AudioPartyScreen> {
  final AudioPartyService _partyService = AudioPartyService();
  ...
  Future<void> _initializeAgora(String channelName) async {
    ...
    await _agoraEngine!.joinChannel(
      ...
      options: const ChannelMediaOptions(
        ...
        publishMicrophoneTrack: true, // Explicitly publish microphone
      ),
    );

    // Start unmuted so participants can hear each other immediately
  ...
  Future<void> _createParty() async {
    ...
    final party = await _partyService.createAudioParty(
    ...
    _partyService.streamParticipants(partyId).listen((participants) {
      if (mounted) {
        setState(() {
          ...
          final myId = _partyService.supabase.auth.currentUser?.id;
          final me = participants.firstWhere(
            (p) => p['user_id'] == myId,
            orElse: () => {'is_muted': true},
          );
  ...
  Future<void> _toggleMute() async {
    ...
    await _agoraEngine!.muteLocalAudioStream(newMutedState);
    await _partyService.toggleMute(_currentParty!['id']);
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Expanded(
            child: Text('Audio Parties', style: AppTheme.headingMedium),
          ),
          IconButton(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text('No active audio parties', style: AppTheme.headingSmall),
          const SizedBox(height: 8),
          const Text(
            'Create one and start talking!',
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Participants', style: AppTheme.headingMedium),
                const SizedBox(height: 16),
  ...
  Widget _buildParticipantCard(Map<String, dynamic> participant) {
    ...
    final myId = _partyService.supabase.auth.currentUser?.id;
    final isMe = participant['user_id'] == myId;
    final isHost = participant['user_id'] == _currentParty?['host_id'];
        ...
        Stack(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                gradient: isMuted ? null : AppTheme.primaryGradient,
                border: Border.all(
                  color: isMuted ? Colors.grey : Colors.transparent,
                  width: 3,
                ),
              ),
              child: Padding(
                padding: const EdgeInsets.all(3),
                child: ClipOval(
                  child: participant['user']?['avatar_url'] != null
                      ? CachedNetworkImage(
                          imageUrl: participant['user']['avatar_url'],
                          fit: BoxFit.cover,
  ...
  void _showCreatePartyDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Create Audio Party', style: AppTheme.headingSmall),
          ...
          child: Column(
            ...
            children: [
              TextField(
                ...
                style: const TextStyle(color: Colors.white),
                decoration: AppTheme.inputDecoration('Room Title'),
              ),
              const SizedBox(height: 16),
              TextField(
