...
class PostService {
  ...
  Future<bool> userHasLiked(String postId, String userId) async {
    final res = await _supabase.from('likes').select('id').eq('post_id', postId).eq('user_id', userId).limit(1) as dynamic;
    if (res == null) return false;
    if (res is List) return res.isNotEmpty;
    if (res is Map && res['data'] is List) return (res['data'] as List).isNotEmpty;
    return false;
  }
  ...
  Future<int> toggleLike(String postId, String userId) async {
    ...
    final cntRes = await _supabase.from('likes').select('id').eq('post_id', postId) as dynamic;
    int count = 0;
    if (cntRes is List) count = cntRes.length;
    else if (cntRes is Map && cntRes['count'] != null) count = cntRes['count'] as int;
  ...
  Future<int> addComment(String postId, String userId, String text) async {
    await _supabase.from('comments').insert({'post_id': postId, 'user_id': userId, 'text': text});
    final cntRes = await _supabase.from('comments').select('id').eq('post_id', postId) as dynamic;
    int count = 0;
    if (cntRes is List) count = cntRes.length;
    else if (cntRes is Map && cntRes['count'] != null) count = cntRes['count'] as int;
